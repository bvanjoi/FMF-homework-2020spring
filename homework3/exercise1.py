...
#
# Try to find solution that satisfies proposition
# (P /\ Q) \/ R
P, Q, R = Bools('P Q R')
# TODO: replace the following line with: F = (P /\ Q) \/ R
'''Fill your code here, replace the following line:'''
print('\nExercise 1-1: (P /\ Q) \/ R')
F = Or(And(P,Q),R)
solve(F)
...
# P \/ (Q \/ R)
P, Q, R = Bools('P Q R')
# TODO: fill code to let 'F' can describe P \/ (Q \/ R)
'''Fill your code here, replace the following line:'''
print('\nExercise 1-1: P \/ (Q \/ R)')
F = Or(P,Q,R)
solve(F)
...
P, Q, R = Bools('P Q R')
# TODO: fill your code here, replace the following line:
print( '\nexerciese 1-2: (P \/ Q) /\ (Q /\ R) /\ (P /\ ~R).')
...
# (P /\ ~S /\ R) /\ (R /\ ( ~P \/ (S /\ ~Q)))
# TODO: fill your code here, replace the following line:
P,S,R,Q = Bools('P S R Q')
print( '\nexercise 1-3: (P /\ ~S /\ R) /\ (R /\ ( ~P \/ (S /\ ~Q)))')
...
# that make the proposition satisfiable, not just the first one. For the
# above example, we want the all first 3 rows.

# Here is the trick: when we get an answer, we negate the
# answer, make a conjunction
# with the original proposition, then check the satisfiability
...
# from z3. Try to get **all solutions** that satisfy the proposition in Exercise 1:
# (P /\ Q) \/ R
P, Q, R = Bools('P Q R')
'''Your code begin, replace the following line:'''
print('\nexercise 1-4:')
F = Or( R, And( P,Q))
solve(F)
F = And(F, Not( And(Not(R), P,Q)))
solve(F)
F = And(F, Not( And(R, P,Q)))
solve(F)
F = And(F, Not( And(R, Not(P),Not(Q))))
solve(F)
F = And(F, Not( And(R, P,Not(Q))))
solve(F)
F = And(F, Not( And(R, Not(P),Q)))
solve(F)
